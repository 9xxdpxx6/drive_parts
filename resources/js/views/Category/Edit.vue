<template>
    <div>
        ...
        <h3 class="mt-2 mb-4">Редактирование категории</h3>
        <div class="row">
...
export default {
    ...
    methods: {
        async getCategory(alias) {
            try {
                const res = await axios.get(`/api/categories/${alias}`)
                ...
                if (this.parentId) {
                    const parentRes = await axios.get(`/api/categories/${this.parentId}`)
                    this.selectedCategory = parentRes.data.data
                }
                if (this.category.preview_image) {
                    const file = { name: this.category.preview_image_name }
                    await this.dropzone.displayExistingFile(file, this.category.preview_image)
                }
            } catch (err) {
        ...
        async update() {
            try {
                ...
                data.append('parent_id', this.selectedCategory ? this.selectedCategory.id : '')

                this.isDataLoaded = false
                await axios.post(`/api/categories/${this.category.alias}`, data)
